...
namespace WeChatPortal.Utils.HttpUtility
{
    ...
    public static class CommonJsonSend
    {
        ...
        /// <param name="callBack"></param>
        /// <returns></returns>
        public static async Task<WxJsonResult> SendAsync(string accessToken, string urlFormat, object data, Action<string> callBack, CommonJsonSendType sendType = CommonJsonSendType.POST, int timeOut = Config.TIME_OUT)
        {
            return await SendAsync<WxJsonResult>(accessToken, urlFormat, data, callBack,sendType, timeOut);
        }
        ...
        /// <returns></returns>
        public static async Task<T> SendAsync<T>(string accessToken, string urlFormat, object data, Action<string> callBack, CommonJsonSendType sendType = CommonJsonSendType.POST, int timeOut = Config.TIME_OUT, bool checkValidationResult = false,
            ...
            )
        {
            try
            {
                ...
                {
                    case CommonJsonSendType.GET:
                        return await Get.GetJsonAsync<T>(url,callBack);
                    case CommonJsonSendType.POST:
                        SerializerHelper serializerHelper = new SerializerHelper();
                        var jsonString = serializerHelper.GetJsonString(data, jsonSetting);
                        using (MemoryStream ms = new MemoryStream())
